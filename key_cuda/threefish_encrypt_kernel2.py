...
with open('threefish.ptx', 'rb') as f:
    ptx_code = f.read()
mod = drv.module_from_buffer(ptx_code)

# Get encryption and decryption kernels
encrypt_kernel = mod.get_function("threefish_encrypt_kernel")
decrypt_kernel = mod.get_function("threefish_decrypt_kernel")
    ...
    for round in reversed(range(ROUNDS)):
        ...
        if (round) % 4 == 0:
            ...
            for i in range(WORDS):
                words[i] = (words[i] - subkeys[s][i]) & 0xFFFFFFFFFFFFFFFF
    return struct.pack("<16Q", *words)
# Precompute subkeys before encryption/decryption
subkeys = generate_subkeys(key, tweak)
...
def encrypt_file(input_file, output_file, key, hmac_key):
    ...
    # Encrypt on GPU
    encrypted_blocks = threefish_gpu_encrypt_blocks(plaintext, subkeys)
...
def threefish_gpu_encrypt_blocks(all_blocks, subkeys):
    """Encrypt ALL blocks on GPU in parallel"""
    blocks_np = np.frombuffer(all_blocks, dtype=np.uint64)
    subkeys_np = np.array(subkeys, dtype=np.uint64).flatten()
    ...
    num_blocks = len(all_blocks) // BLOCK_SIZE
    threads_per_block = 256
    grid_size = (num_blocks + threads_per_block - 1) // threads_per_block
    ...
    # Retrieve results
    encrypted_blocks = np.empty_like(blocks_np)
    drv.memcpy_dtoh(encrypted_blocks, blocks_gpu)
    return encrypted_blocks.tobytes()
# Add to existing code
decrypt_kernel = mod.get_function("threefish_decrypt_kernel")

def threefish_gpu_decrypt_blocks(all_blocks, subkeys):
    """Decrypt all blocks on GPU in parallel"""
    blocks_np = np.frombuffer(all_blocks, dtype=np.uint64)
    subkeys_np = np.array(subkeys, dtype=np.uint64).flatten()
    ...
    # Retrieve results
    decrypted_blocks = np.empty_like(blocks_np)
    drv.memcpy_dtoh(decrypted_blocks, blocks_gpu)
    return decrypted_blocks.tobytes()
...
def decrypt_file(input_file, key, hmac_key):
    with open(input_file, 'rb') as f_in:
        ...
        # Decrypt on GPU
        subkeys = generate_subkeys(key, tweak_bytes)
        decrypted_data = threefish_gpu_decrypt_blocks(encrypted_blocks, subkeys)

        # Remove padding (ensure valid padding length)
        ...
        # Save output
        output_file = os.path.join(os.path.dirname(input_file), "dec_" + os.path.basename(input_file).replace(".enc", ""))
        with open(output_file, 'wb') as f_out:
